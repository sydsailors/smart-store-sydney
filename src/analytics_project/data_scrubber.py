...
class DataScrubber:
    ...
    def handle_missing_data(self, drop=False, fill_value=None) -> pd.DataFrame:
        before = self.df.isnull().sum().sum()
        if drop:
            self.df = self.df.dropna()
            logger.info(f"Dropped rows with missing data. Remaining rows: {len(self.df)}")
        elif fill_value is not None:
            self.df = self.df.fillna(fill_value)
            logger.info(f"Filled {before} missing values with '{fill_value}'")
        return self.df
    ...
    def remove_duplicate_records(self) -> pd.DataFrame:
        ...
        after = len(self.df)
        logger.info(f"Removed {before - after} duplicate records. Remaining rows: {after}")
        return self.df
...
if __name__ == "__main__":
    ...
    data = {"A": [1, 2, 2, None], "B": [" x ", "y ", "y ", None]}
    ...
    logger.info("Data cleaning complete. Logged results to project.log.")
